...
function Arts_Marksheet(props) {
	...
	const [getArtsHindiTheory, setArtsHindiTheory] = React.useState("")
	const [getArtsHindiPractical, setArtsHindiPractical] = React.useState("")
	...
	const readAllRecords = () => {
		if (!isUndefined(props.location.res)) {
			...
			setDob(res.dob)
			setAdharno(res.adharno)
			setSssmidno(res.sssmidno)
			setEnrollment(res.enrollment)
			let maxLength = 26;
			let address = (res.address).substring(0, maxLength) + '...';

			setAddress(address)

			setPhoto(`${BaseUrl}/images/${res.photo}`)
			...
			if (res.examtype == "quarterly") {
				setExamType("Quarterly")
			} else if (res.examtype == "halfyearly") {
				setExamType("Halfyearly")
			} else if (res.examtype == "annually") {
				setExamType("Annually")
			}

			if (res.class == "11") {
				setClass("11th")
			} else if (res.class == "12") {
				setClass("12th")
			}
			...
			let body = {
				...
				studentmedium: res.medium
			}

			axios.post(`${BaseUrl}/lockpostadd/GetRankForArtsStudent`, body)
				.then((rankRes) => {
					for (var i = 0; i < rankRes.data.length; i++) {
						if (rankRes.data[i].studentid == res.studentid) {
							setRank(rankRes.data[i].Rank)
						}
					}
				})
				.catch((err) => {
			...
			<div className="stl_ stl_02">
				...
				<div className="stl_view">
					<div className="stl_05 stl_06">
						...
						<div className="stl_01" style={{ left: "13.9758em", top: rowTops[1] }}>
							<span className="stl_32 stl_11 stl_40">20 &nbsp;</span>
						</div>
						{electiveRows.map((row, i) => (
							<div className="stl_01" style={{ left: "13.9758em", top: rowTops[i + 2] }} key={"assignmax" + i}>
								<span className="stl_32 stl_11 stl_40">{row.practicalMax} &nbsp;</span>
							</div>
						))}
						...
						{/* Assignment(Practical) Obtained column */}
						<div className="stl_01" style={{ left: "20.682em", top: rowTops[0] }}>
							<span className="stl_32 stl_11 stl_40">{getArtsEnglishPractical} &nbsp;</span>
						</div>
						<div className="stl_01" style={{ left: "20.682em", top: rowTops[1] }}>
							<span className="stl_32 stl_11 stl_40">{getArtsHindiPractical} &nbsp;</span>
						</div>
						{electiveRows.map((row, i) => (
							<div className="stl_01" style={{ left: "20.682em", top: rowTops[i + 2] }} key={"assignobt" + i}>
								<span className="stl_32 stl_11 stl_40">{row.practical} &nbsp;</span>
							</div>
						))}
						...
						<div className="stl_01" style={{ left: "27.1758em", top: rowTops[1] }}>
							<span className="stl_32 stl_11 stl_40">80 &nbsp;</span>
						</div>
						{electiveRows.map((row, i) => (
							<div className="stl_01" style={{ left: "27.1758em", top: rowTops[i + 2] }} key={"theorymax" + i}>
								<span className="stl_32 stl_11 stl_40">{row.theoryMax} &nbsp;</span>
							</div>
						))}

						<div className="stl_01" style={{ left: "27.1758em", top: "42.2952em" }}>
							<span className="stl_32 stl_11 stl_40">
								{80 * 2 + electiveRows.reduce((sum, r) => sum + r.theoryMax, 0)} &nbsp;
							</span>
						</div>
						...
						{/* Theory Obtained column */}
						<div className="stl_01" style={{ left: "33.682em", top: rowTops[0] }}>
							<span className="stl_32 stl_11 stl_40">{getArtsEnglishTheory} &nbsp;</span>
						</div>
						<div className="stl_01" style={{ left: "33.682em", top: rowTops[1] }}>
							<span className="stl_32 stl_11 stl_40">{getArtsHindiTheory} &nbsp;</span>
						</div>
						{electiveRows.map((row, i) => (
							<div className="stl_01" style={{ left: "33.682em", top: rowTops[i + 2] }} key={"theoryobt" + i}>
								<span className="stl_32 stl_11 stl_40">{row.theory} &nbsp;</span>
							</div>
						))}

						<div className="stl_01" style={{ left: "33.682em", top: "42.2952em" }}>
							<span className="stl_32 stl_11 stl_40">
								{getArtsEnglishTheory + getArtsHindiTheory + electiveRows.reduce((sum, r) => sum + r.theory, 0)} &nbsp;
							</span>
						</div>
						...
						{electiveRows.map((row, i) => (
							<div className="stl_01" style={{ left: "4.982em", top: rowTops[i + 2] }} key={"label" + i}>
								<span className="stl_52 stl_11 stl_29" style={{ textTransform: "uppercase" }}>
									{row.label} &nbsp;
								</span>
							</div>
						))}
						...
						{/* per-subject grade (Theory+Practical, pass/fail/distinction) */}
						<div className="stl_01" style={{ left: "40.8283em", top: rowTops[0] }}>
							<span className="stl_32 stl_11 stl_54">{getEnglish} &nbsp;</span>
						</div>
						<div className="stl_01" style={{ left: "40.8283em", top: rowTops[1] }}>
							<span className="stl_32 stl_11 stl_54">{getHindi} &nbsp;</span>
						</div>
						{electiveRows.map((row, i) => (
							<div className="stl_01" style={{ left: "40.8283em", top: rowTops[i + 2] }} key={"grade" + i}>
								<span className="stl_32 stl_11 stl_54">{row.grade} &nbsp;</span>
							</div>
						))}
